...
export class MockAIService implements AIService {
  ...
  async suggestStateTransition(
    ...
    // Simple heuristic: if recent message contains "PO", suggest PO received
    const recentMessage = messages[messages.length - 1];
    if (recentMessage?.content.toLowerCase().includes("po")) {
      return "PO received";
    }
    return null;
  }
